...
std::string file_path = std::string("exela-config.ini");
struct Config {
	...
	ImVec4 skeleton = ImVec4(0.0f, 1.0f, 1.0f, 1.0f);
	ImVec4 distance = ImVec4(0.0f, 1.0f, 1.0f, 1.0f);
...
void SaveConfig() {
	...
	config_file << config.filledbox << std::endl;
	config_file << config.deathmatch << std::endl;
	...
	config_file << config.distance.x << " " << config.distance.y << " " << config.distance.z << " " << config.distance.w << std::endl;
	config_file.close();
}
...
void LoadConfig() {
	...
	config_file >> config.filledbox;
	config_file >> config.deathmatch;
	...
	config_file >> config.distance.x >> config.distance.y >> config.distance.z >> config.distance.w;
	config_file.close();
...
{	
	...
	// controllers
	std::ptrdiff_t damage_handler = 0xa08;			// world > game_instance > localplayers_array[0] > playercontroller > apawn > damage_handler			// aactor > damage_controller

	// camera
	std::ptrdiff_t cameracache = 0x1FB0;		        // world > game_instance > localplayers_array[0] > playercontroller > camera_controller > camera_rotation
	std::ptrdiff_t camera_manager = 0x480;			// world > game_instance > localplayers_array[0] > playercontroller > camera_controller > camera_position
	...
	std::ptrdiff_t team_component = 0x630;		                  // world > persistent_level > aactor > player_state > team_component
	...
	std::ptrdiff_t player_state = 0x3F8;				  // world > persistent_level > aactor > player_state

	// mesh
	std::ptrdiff_t control_rotation = 0x448;
	std::ptrdiff_t mesh = 0x438;					// world > persistent_level > aactor > mesh
...
char* espstring = XorString("Box ESP");
std::string sw = XorString("\n Please open the game and press F1 in the Lobby..");
std::string activecheat = XorStr("[+] Cheat active. Good luck.");
std::string notfounddrver = XorStr("[-] Driver not found. Exiting..");
std::string ntfoundgame = XorStr("[-] Please open the game..");
